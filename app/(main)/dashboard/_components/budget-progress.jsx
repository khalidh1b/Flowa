                    ...
                    <div className='flex items-center gap-2 mt-1'>
                        {
                            isEditing ? (<div className='flex items-center gap-2'>
                                ...
                                <Button variant="ghost" size="icon" onClick={handleBudgetUpdate} ><Check className='h-4 w-4 text-green-500' disabled={isLoading} /></Button >
                                <Button variant="ghost" size="icon" onClick={handleCancel} ><X className='h-4 w-4 text-red-500' disabled={isLoading} /></Button>
                            </div>) : (
                                <>
                                    <CardDescription>
                                        {initialBudget ? `${currency} ${currentExpenses.toFixed(2)} of ${currency} ${initialBudget.amount.toFixed(2)} spent` : "No budget set"}
                                    </CardDescription>
